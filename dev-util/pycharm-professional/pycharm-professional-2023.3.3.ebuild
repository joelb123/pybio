...
src_prepare() {
	...
	local remove_me=(
		...
		plugins/remote-dev-server/selfcontained/lib/libdbus-1.so.3
		plugins/remote-dev-server/selfcontained/lib/libgio-2.0.so.0
		plugins/remote-dev-server/selfcontained/lib/libglapi.so.0
		plugins/remote-dev-server/selfcontained/lib/libkrb5support.so.0
		plugins/remote-dev-server/selfcontained/lib/libsystemd.so.0
		...
		plugins/tailwindcss/server/tailwindcss-language-server
	)
...
src_install() {
	...
	fperms 755 ${dir}/bin/{format.sh,fsnotifier,inspect.sh,ltedit.sh,pycharm.sh}

	fperms 755 ${dir}/${jre_dir}/bin/{java,javac,jcmd,jdb,jfr,jhsdb,jinfo,jmap,jps,jrunscript,jstack,jstat,keytool,rmiregistry,serialver}
